...
func (this CipherBlockCBC) Encrypt(password, plaintext []byte) ([]byte, []byte, error) {
    ...
    key, iv := derivedKey(string(password), string(salt), this.iterationCount, this.keySize, this.hashFunc)
...
func (this CipherBlockCBC) Decrypt(password, params, ciphertext []byte) ([]byte, error) {
    ...
    key, iv := derivedKey(string(password), string(param.Salt), param.IterationCount, this.keySize, this.hashFunc)
