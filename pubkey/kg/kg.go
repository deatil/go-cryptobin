...
func SignUsingKToRS(k *big.Int, priv *PrivateKey, hashFunc Hasher, msg []byte) (r, s *big.Int, err error) {
    ...
    eBuf = formatMessage(eBuf, bitLen)
...
func VerifyWithRS(pub *PublicKey, hashFunc Hasher, msg []byte, r, s *big.Int) (bool, error) {
    ...
    // Compute u = r * w mod q
    u := new(big.Int).Mul(r, w)
    u.Mod(u, N)

    // Compute v = e * w mod q
    v := new(big.Int).Mul(e, w)
    v.Mod(v, N)

    // Compute r' = u * Q  + v * G
    x21, y21 := curve.ScalarMult(pub.X, pub.Y, u.Bytes())
    x22, y22 := curve.ScalarBaseMult(v.Bytes())
    x2, _ := curve.Add(x21, y21, x22, y22)

    // Compute r' = W'_x mod q
    rPrime := x2.Mod(x2, N)
